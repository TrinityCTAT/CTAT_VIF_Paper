...
files = list.files("data/HNSC/", pattern = "bedgraph")
for (file in files) {
    message("processing file: ", file)
    path = paste0("data/HNSC/", file)
...
write.table(alldata, "HNSC.virus_coverage_info.tsv", sep="\t", quote=F)
...
virus_data =  alldata %>% filter(chrom == "HPV16")
...
p = virus_data %>% ggplot(aes(x=lend, y=samplename, fill=log2(coverage+1))) + geom_tile() + ggtitle("HPV16") +
...
pdf("HNSC_HPV_genome_coverage.pdf")
...
for (virus in viruses) {
  ...
  virus_coverage_matrix_filename = paste0("HNSC.", virus, ".coverage.matrix")
  ...
  sample_insertion_labeling_filename = paste0("HNSC.", virus, ".insertion_labeling.tsv")
