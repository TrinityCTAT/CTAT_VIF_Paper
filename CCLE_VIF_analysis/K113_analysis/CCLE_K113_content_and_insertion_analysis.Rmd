...
p_sample_counts = filtered_virus_content_data %>% select(tissue_id, sample_name) %>% group_by(tissue_id) %>% tally() %>% ggplot(aes(x=tissue_id, y=n)) + geom_col() + 
...
p_mapped_dist = filtered_virus_content_data %>% select(tissue_id, mapped) %>% ggplot(aes(x=tissue_id, y=mapped)) + geom_boxplot()  +
...
write.table(merged_distilled_report, file='CCLE.filtered_K113_content_and_insertions.distilled.tsv', quote=F, sep="\t", row.names=F)
...
ggsave(summary_frac_insertion_virus_table_plot, file="CCLE_K113_summary_plot.pdf", width=12, height=12)
...
```{r}
ggsave(virus_median_mapped_all_plot, file="CCLE_K113_median_mapped_all_plot.pdf", width=20, height=9)
...
filtered_insertions_data %>% filter(is_primary != "False") %>% select(sample_name, virus_brkend_grp, splice_type) %>% unique() %>% group_by(virus_brkend_grp, splice_type) %>% tally() %>% arrange(desc(n))
...
```{r}
filtered_insertions_data %>% filter(is_primary == "True")  %>% select(sample_name, contig, total)
