...
insertions_data = read.table(gzfile("data/CCLE.virus_insertions.tsv.gz"), sep="\t", stringsAsFactors = F, header=T, com='')

insertions_data = insertions_data %>% rename(sample_name = X.sample)

insertions_data = insertions_data %>% filter(total > 0)

insertions_data$tissue_id = sapply(insertions_data$sample_name, function(x) { str_split(x, "_", n=2, simplify=FALSE)[[1]][2]} )
...
```{r}
virus_content_data = read.table(gzfile("data/CCLE.virus_content.tsv.gz"), header=T, sep="\t", stringsAsFactors = F, com='')

virus_content_data = virus_content_data %>% rename(sample_name = X.sample)

virus_content_data$tissue_id = sapply(virus_content_data$sample_name, function(x) { str_split(x, "_", n=2, simplify=FALSE)[[1]][2]} )
...
ggsave(summary_frac_insertion_virus_table_plot, file="CCLE_virus_summary_plot.pdf", width=12, height=12)
...
```{r}
ggsave(virus_median_mapped_all_plot, file="CCLE_virus_median_mapped_all_plot.pdf", width=20, height=9)
...
ggsave(virus_median_mapped_by_tissue_id_plot, width=20, height=35, file="CCLE_virus_median_mapped_by_tissue_id.plot.pdf")
