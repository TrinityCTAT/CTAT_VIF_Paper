...
task RunViFi {
    ...
    command <<<
        ...
        tar -czf ~{sample_id}_OUTPUT.tar.gz ~{sample_id}

    >>>

    output {
        File output_file="~{sample_id}_OUTPUT.tar.gz"
        File output_clusters_txt_range="./~{sample_id}/~{sample_id}.clusters.txt.range"
        File output_clusters_txt = "./~{sample_id}/~{sample_id}.clusters.txt"
    }
