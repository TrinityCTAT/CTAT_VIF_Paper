...
class ReformatBatVI:
    ...
    def __init__(   self, 
                    ...
                    Virus,
                    output,
                    sample_tag = "test"): # arguments to class instantiation 
    ...
    def reformat( self ):
        ...
        # Rename the columns 
        df = df.rename({"Chr":"chrA","Human Pos":"coordA","Sign":"orientA","Viral Sign":"orientB", "Viral Pos":"coordB", "Read Count":"total"},
                       axis='columns')
        ...
        entry = df["chrA"] + "~" + df["coordA"].astype("str") + "~" + df["chrB"] + "~" + df["coordB"].astype("str")
        df.insert(1, "entry", entry)


        # Reorder columns 
        column_order = ['#sample','entry','chrA','coordA','orientA','chrB','coordB','orientB','total','Split Reads','Uniquely Mapped Reads','Multiply Mapped Reads','Rank1 Hits']
        df = df[column_order]
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Reformat  BatVI output for benchamrking.", 
    ...
    BatVI = ReformatBatVI(        input_file = pred_insertions_filename,
                                        Virus      = virus,
