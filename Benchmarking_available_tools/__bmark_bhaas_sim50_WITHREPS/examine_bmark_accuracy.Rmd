...
data %>% filter(min_reads == MIN_READS) %>% 
  ggplot(aes(x=method, y=recall, fill=method)) + geom_col() + facet_wrap(~virus) + 
...
data %>% filter(min_reads == MIN_READS) %>%
  ggplot(aes(x=method, y=precision, fill=method)) + geom_col() + facet_wrap(~virus) + 
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  ggtitle("Precision")
