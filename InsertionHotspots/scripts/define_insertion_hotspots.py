...
def main():
    ...
    parser = argparse.ArgumentParser(description="define hotspot insertions", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    
    parser.add_argument("--insertions_tsv", type=str, required=True, help="define insertions hotspots")

    parser.add_argument("--min_hotspot_samples", type=int, default=2, help="min sample count to define hotspot")

    parser.add_argument("--min_ev_reads", type=int, default=5, help="min number of evidence reads for each insertion candidate")
    
    parser.add_argument("--window_size", type=int, required=False, default=1000000, help="window in which to define hotspots")
    parser.add_argument("--debug", action='store_true', default=False, help="debug mode")

    parser.add_argument("--output", type=str, required=True, help="output with insertions assigned to hotspots")
    ...
    if min_samples_per_hotspot > 1:
        data_w_hotspots = data_w_hotspots[data_w_hotspots['hotspot_sample_counts'] >= min_samples_per_hotspot]
...
def define_virus_and_genome_info(row):
    ...
    assert ((re.match("^chr[\\dMXY]+", chrA) is not None) ^ (re.match("^chr[\\dMXY]+", chrB) is not None)), "Error, cannot figure out which of pair is human chromosome"
...
def find_hotspots(chrom_df, window_size):
    ...
    for i, row in chrom_df.iterrows():
        ...
        if windowed_rows:
            while len(windowed_rows) > 0 and human_coord - windowed_rows[0]['human_coord'] > window_size:
                windowed_rows.pop(0)
    ...
    for hotspot_candidate in sample_tallies:
        ...
        if not within_range_of_hotspot(hotspot_candidate, selected_hotspots, window_size):
    ...
    hotspot_df = pd.DataFrame( { 'Chromosome': [human_chrom] * len(hotspot_coordinates),
