...
def main():
    ...
    parser.add_argument("--hotspots", type=str, required=True, help="filename hotspot_sample_data.threshold=NUM.tsv")
    parser.add_argument("--ref_gene_spans", type=str, required=True, help='ref annot gene spans file')
    parser.add_argument("--num_genes_include", type=int, default=3, help='number of genes to include on both sides of the breakpoint')
    parser.add_argument("--no_gene_decoration", action='store_true', default=False, help='do not highlight any gene insertion')
    ...
    def annotate_neighboring_genes_for_row(row):
        ...
        left_genes, right_genes = annotate_neighboring_genes(chr, coord, chr_to_ordered_gene_list[chr], num_genes_include,  no_gene_decoration_flag)
    ...
    hotspots_df = hotspots_df.sort_values(['Chromosome', 'Start_b'])
    hotspots_df = hotspots_df[ ['sample', 'contig', 'human_chrom', 'human_coord', 'virus_genome', 'virus_coord', 'total', 'hotspot', 'hotspot_sample_counts', 'left_genes', 'right_genes'] ]
...
def annotate_neighboring_genes(chrom, coord, ordered_gene_list, num_genes_include, no_gene_decoration_flag):
