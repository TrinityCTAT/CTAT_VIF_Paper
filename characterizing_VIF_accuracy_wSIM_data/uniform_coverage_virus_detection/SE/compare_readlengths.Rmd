...
sim50_data = get_TP_FP_per_virus("sim50_SE/prelim_insertion_mappings.tsv")
sim100_data = get_TP_FP_per_virus("sim100_SE/prelim_insertion_mappings.tsv")
sim150_data = get_TP_FP_per_virus("sim150_SE/prelim_insertion_mappings.tsv")
...
NUM_TRUTH_ENTRIES = nrow(read.table(gzfile("sim50_SE/data/insertion_truth_set.tsv.gz"), header=T))
...
accuracy_stats %>%
    ...
    geom_point() + geom_line(aes(groups=readlen)) + ggtitle("all 960 human viruses, 50 insertions each") +
...
accuracy_stats_HPV %>% 
    ggplot(aes(x = recall, y=precision, color=factor(readlen))) +
    geom_point() + geom_line(aes(groups=readlen)) + ggtitle("143 HPV strains only, 50 insertions each") +
    geom_text(aes(label=min_reads),hjust=0, vjust=0)
