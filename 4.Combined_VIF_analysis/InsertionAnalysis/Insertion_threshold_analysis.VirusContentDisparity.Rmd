...
# see Merge_Virus_Insertions_and_Content.Rmd for generating file below:
...
insertions_data  = insertions_data %>% mutate(insertion_locus = paste0(humanchr, "^", round(human_coord/BINDIST)*BINDIST))

num_insertion_loci = insertions_data %>% select(sample_id, virus, insertion_locus) %>% unique() %>% nrow()
...
```{r}
num_insertions_with_matching_virus_content = insertions_data %>%  filter(matching_virus_content) %>% select(sample_id, virus, insertion_locus) %>% 
  unique() %>% nrow()
message("num insertion loci with matching virus content: ", num_insertions_with_matching_virus_content)

num_insertions_without_matching_virus_content = insertions_data %>% filter(! matching_virus_content) %>% select(sample_id, insertion_locus) %>% 
  unique() %>% nrow()

message("num insertion loci without matching virus content: ", num_insertions_without_matching_virus_content)

frac_loci_without_matching_virus = num_insertions_without_matching_virus_content / num_insertion_loci

message("frac insertion loci without matching virus content: ", frac_loci_without_matching_virus)
...
# min_read_count   num_insertions  num_samples_with_insertions  frac_without_matching_virus
#     1	               37184	           1323	                 1.1%
#     2	               19031	           1083	                 1.3%
...
```{r}
# what samples have the insertions detected?

insertions_data %>% filter(total >=1) %>% select(sample_name, tissue) %>% unique() %>% group_by(tissue) %>% tally() %>% arrange(desc(n))
...
# counting samples with insertions
