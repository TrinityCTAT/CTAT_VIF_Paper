...
spliced_insertions %>% filter(grepl("HPV", virus)) %>% nrow()
...
# simplify a table for supplementary materials.

spliced_insertions_report = spliced_insertions %>% select(sample_name, contig,virus,  splice_type, virus_brkend_grp, tumor_or_normal, total_rpm, Gene_ID, project, cohort) 

write.table(spliced_insertions_report, file='spliced_insertions_report.tsv', quote=F, sep="\t", row.names=F)

spliced_insertions_report
```
...
# Viruses that dominate among chimerically spliced fusions include HPV (59%), EBV (15%), and HBV (11%). 
...
spliced_insertions %>% filter(virus == "EBV") %>% select(sample_id, project, cohort, Gene_ID) %>% unique() %>%
...
spliced_insertions %>% filter(virus=="EBV") %>% select(sample_id, virus_brkend_grp,  human_coord) %>% unique() %>% 
  group_by(virus_brkend_grp) %>%  tally() %>% arrange(desc(n)) %>% ungroup() %>% mutate(n_frac=prop.table(n))
