...
data_1e5 %>% ggplot(aes(x=as.factor(hotspot_size), y=num_hotspots)) + geom_boxplot() +
  ggtitle("distribution of counts of hotspot sizes (win=1e5")
...
data_5e5 = read.table(gzfile("data/rand_cluster_size_counts.win_5e5.tsv.gz"), header=F, sep="\t")

colnames(data_5e5) = c('trial', 'hotspot_size', 'num_hotspots')

data_5e5 = data_5e5 %>% filter(hotspot_size > 1)


data_5e5 %>% ggplot(aes(x=as.factor(hotspot_size), y=num_hotspots)) + geom_boxplot() +
  ggtitle("distribution of counts of hotspot sizes (win=5e5)")
...
data_1e6 %>% ggplot(aes(x=as.factor(hotspot_size), y=num_hotspots)) + geom_boxplot() +
  ggtitle("distribution of counts of hotspot sizes (win=1e6)")
...
combined_data %>%  ggplot(aes(x=as.factor(hotspot_size), y=num_hotspots, color=winsize)) + geom_boxplot(outlier.shape = NA) +
...
frac_info  = combined_data %>% select(trial, hotspot_size) %>% unique() %>%
