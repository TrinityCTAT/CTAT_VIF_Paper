...
hotspot_insertions = read.table("../hotspots.win_1e5.tsv.w_20_neighbors.regrouped_by_insert_gene.gz", header=T, sep="\t", stringsAsFactors = F)
...
FHIT_insertions %>% select(sample, sample_id, is_primary, seqtype, virus_genome, human_coord, virus_coord, total, total_rpm)
...
FHIT_insertion_matrix = FHIT_insertions %>% mutate(insertion_coord = paste0(virus_genome, ":", virus_coord, "^", human_coord)) %>%
...
```{r}
pdf("FHIT_hotspot_insertions.pdf", width=9, height=10)
pheatmap(m, na_col = "gray", cluster_rows=hotspot_clust, cluster_cols=sample_clust)
dev.off()
