...
fractional_cluster_att_representation = FI_fusions_data %>% 
  # restrict this to dominant isoform per fusion
...
# 76 samples with insertions and fusions
...
df_A_with_nearest_df_B = function(df_A, df_B) {
    ...
    gr_df_A = with(df_A, GRanges(Chromosome_val,
                                IRanges(start=Start_val, end=End_val+1)))
    
    gr_df_B = with(df_B, GRanges(Chromosome_val,
                                IRanges(start=Start_val, end=End_val+1)))

    n_hits = nearest(gr_df_A, gr_df_B, select='all', ignore.strand=T)
  
   
    
    df_A_with_nearest_df_B = bind_cols(df_A[from(n_hits),] %>% select(-Chromosome_val, -Start_val, -End_val), 
                                       df_B[to(n_hits),] %>% select(-Chromosome_val, -Start_val, -End_val) )
...
# ensure the chromsome factors match

all_chroms = union(FI_fusions_coords$Chromosome_val, insertions_data$Chromosome_val)

insertions_data$Chromosome_val = factor(insertions_data$Chromosome_val, levels=all_chroms)

FI_fusions_coords$Chromosome_val = factor(FI_fusions_coords$Chromosome_val, levels=all_chroms)
...
# how many fusion transcript instances mapped to insertion loci
fusion_mapped_insertions %>% select(sample_id, fusion_at_insertion) %>% unique() %>% nrow()

fusion_mapped_insertions %>% select(sample_id) %>% unique() %>% nrow()

# 165 fusions near insertion loci at 68 samples 
...
# what fusions are we finding in cell lines from CCLE?
```{r}

fusion_mapped_insertions  %>% filter(grepl("CCLE", sample_name)) %>%
    select(sample_name, virus, fusion_at_insertion, Annots)
