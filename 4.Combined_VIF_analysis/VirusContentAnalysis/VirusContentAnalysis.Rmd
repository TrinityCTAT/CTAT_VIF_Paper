...
## Group all HPV strains as HPV for simpler summary statistics on HPV
# - comment out the line below for individual strain stats

# virus_content = virus_content %>% mutate(virus = ifelse(grepl("HPV", virus), "HPV", virus))
...
sample_virus_counts = virus_content %>% 
  filter(mapped >= 10 & n_bases_covered >= 500 & mapped_rpm >= 10) %>% 
...
sample_type_virus_fractions_summary_stats = sample_type_virus_fractions %>% mutate(project = paste(project, tumor_or_normal, seqtype)) %>% arrange(desc(frac_virus))  %>%
  select(cohort, project, seqtype, sample_count, sample_virus_counts, frac_virus, virus)

sample_type_virus_fractions_summary_stats
...
sample_type_virus_fractions_summary_stats %>% filter(grepl("CESC", project))
...
sample_type_virus_fractions_summary_stats %>% filter(grepl("HNSC", project)) %>% filter(grepl("HPV", virus))

```

```{r}

sample_type_virus_fractions_summary_stats %>% filter(grepl("LIHC", project)) %>% filter(virus=="HBV")
...
sample_type_virus_fractions_summary_stats %>% filter(grepl("STAD", project)) %>% filter(virus=="EBV")
...
sample_type_virus_fractions_summary_stats %>% filter(grepl("EBV", project)) %>% filter(virus=="EBV")
...
virus_content %>% filter(virus == "EBV") %>% filter(mapped >= 10 & n_bases_covered >= 500 & mapped_rpm >= 10) %>% filter(project=="STAD") %>% select(seqtype, sample_id) %>% unique() %>% group_by(sample_id) %>% tally() %>% group_by(n) %>% tally()
...
# rare infectino of HSV1 in X4EP brain samples detected.
...
# SARC-MJ-A68H^SARC^tumor	HPV51	7808	65297	468	5653	0.724	SARC


# 1KXAM^Skin_Not_Sun_Exposed_Suprapubic^normal	HPV15	7413	8846	12	4996	0.674	Skin_Not_Sun_Exposed_Suprapubic
# 1KXAM^Skin_Not_Sun_Exposed_Suprapubic^normal	HPV80	7427	8655	167	5483	0.738	Skin_Not_Sun_Exposed_Suprapubic
...
virus_content %>% filter(virus == "NC_001802_9181nt_Human_immunodeficiency_virus_1") %>% filter(n_bases_covered >= 500) %>%
  select(cohort, project, tumor_or_normal, participant) %>%
  group_by(cohort, project, tumor_or_normal) %>% tally() %>% arrange(desc(n))
...
virus_content %>% filter(virus == "NC_001802_9181nt_Human_immunodeficiency_virus_1") %>% filter(n_bases_covered >= 500) %>%
  filter(grepl("AML", project)) %>% 
  select(cohort, project, tumor_or_normal, participant) %>% group_by(participant) %>% tally()

# only a small number are tumor/normal matched showing up with HIV trace levels
...
# 46 UCEC RNA-seq samples with HPV38 detected and only 2 above 10 rpm
