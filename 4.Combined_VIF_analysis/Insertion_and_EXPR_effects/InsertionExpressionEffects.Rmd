...
randomized_data = read.table(gzfile("virus_insertions_unfiltered.10kb.expression_region_analysis.RANDOMIZED.tsv.gz"), header=T, sep="\t", stringsAsFactors = F)
...
p = insertions_w_expression_data %>% 
   
    ggplot(aes(x=expr_quantile)) + geom_density( fill='blue', alpha=0.5) 

p = p + geom_density(data=randomized_data, fill='yellow', alpha=0.5, )

p +  facet_wrap(~virus, scale='free_y')
...
viruses_subset = c("Eppstein-Barr", "HBV", "HPV16", "HPV18")

p = insertions_w_expression_data %>% 
    filter(virus %in% viruses_subset) %>%
    ggplot(aes(x=expr_quantile)) + theme_bw() + geom_density( fill='blue', alpha=0.5) 

p = p + geom_density(data=randomized_data %>% filter(virus %in% viruses_subset), 
...
```{r}
combined_data %>% filter(type=='real') %>% select(sample_id, virus) %>% group_by(virus) %>% tally() %>% arrange(desc(n))
```
