...
## Curie Institute samples hyb capture

```{r}

Curie_hybcap_virus_content = read.table("../InstitutCurie/virus_content.tsv", header=T, sep="\t", stringsAsFactors = F)

Curie_hybcap_virus_content = Curie_hybcap_virus_content  %>% 
    rename(tissue = tissue_id)  %>%
    ...
    mutate(seqtype = "HYB", 
           ...
           cohort='Curie') %>% select(-DOI)


Curie_hybcap_virus_content %>% head()
```
...
combined_all_virus_content = combined_all_virus_content %>% 
  mutate(virus = ifelse(virus=="NC_007605_171823nt_Human_gammaherpesvirus_4" | virus == "NC_009334_172764nt_Human_herpesvirus_4_type_2", "EBV", virus)) %>%
...
combined_all_virus_content %>% filter(participant == "X4EP" & sample_name == "GTEX-X4EP-0011-R10B-SM-4QASJ")
...
EBV_virus_content = combined_all_virus_content %>% filter(virus == "EBV")
...
EBV_virus_content = EBV_virus_content %>%
  ...
  arrange(desc(mapped)) %>% filter(row_number()==1) %>%
  ungroup() %>%
  mutate(mapped = sum_mapped, n_bases_covered = max_n_bases_covered, frac_covered = max_frac_covered,
...
combined_all_virus_content = bind_rows(combined_all_virus_content %>% filter(virus != "EBV"),
                                       EBV_virus_content)
