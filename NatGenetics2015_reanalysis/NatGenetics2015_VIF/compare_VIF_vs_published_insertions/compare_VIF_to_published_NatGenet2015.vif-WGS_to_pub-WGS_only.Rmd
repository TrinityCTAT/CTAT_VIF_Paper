...
published_WGS_insertions = read.table("../../NatGenetics2015_reanalysis_published_data/data/TableS3.WGS_insertion_sites.txt", header=T, sep="\t", stringsAsFactors = F) %>% 
...
vif_WGS_insertions = read.table("../insertions.tsv", header=T, sep="\t", stringsAsFactors = F) %>% filter(sample_type == "wgs")

vif_WGS_insertions$sample_id = sapply(vif_WGS_insertions$sample_label, function(x) { str_split(x, "-")[[1]][4]})
...
sample_ids = intersect(published_WGS_insertions %>% select(sample_id) %>% unique() %>% pull(sample_id),
                       vif_WGS_insertions %>% select(sample_id) %>% unique() %>% pull(sample_id))
...
for (selected_sample_id in sample_ids) {
  ...
  vif_WGS_insertions_for_sample = vif_WGS_insertions %>% filter(sample_id == selected_sample_id) %>% select(-sample_id) %>%
  ...
  vif_with_nearest_published = bind_rows(vif_with_nearest_published, 
                                            df_A_with_nearest_df_B(vif_WGS_insertions_for_sample, published_WGS_insertions_for_sample) )
  
  
  published_with_nearest_prelim = bind_rows(published_with_nearest_prelim,
                                            df_A_with_nearest_df_B(published_WGS_insertions_for_sample, vif_WGS_insertions_for_sample))
