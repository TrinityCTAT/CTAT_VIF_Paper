...
prelim_insertions = read.table("../insertions.tsv", header=T, sep="\t", stringsAsFactors = F) %>% filter(sample_type == "hivid")
...
sample_ids = intersect(published_insertions %>% select(sample_id) %>% unique() %>% pull(sample_id),
                       prelim_insertions %>% select(sample_id) %>% unique() %>% pull(sample_id))
