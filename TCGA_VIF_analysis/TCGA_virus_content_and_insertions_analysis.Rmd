...
REMOVE_LIKELY_CONTAMINANTS = TRUE
...
# get info on contaminants
contaminants_info = read.table("Find_Likely_Contaminants/TCGA_VIF_contaminants_only.tsv.saved", header=T, sep="\t", stringsAsFactors = F) %>% select(sample_name, virus, CONTAM) %>% unique()
...
insertions_data = read.table(gzfile("data/TCGA.virus_insertions.tsv.gz"), sep="\t", stringsAsFactors = F, header=T, com='')
...
# restrict to PE read samples:
insertions_data = insertions_data %>% filter(sample_name %in% PE_samples)
...
# remove contaminants in filtering

nrow(insertions_data)

if (REMOVE_LIKELY_CONTAMINANTS) {

  insertions_data = left_join(insertions_data, contaminants_info, by=c('sample_name', 'virus')) %>%  filter(is.na(CONTAM)) %>% select(-CONTAM)
...
```{r}
virus_content_data = read.table(gzfile("data/TCGA.virus_content.tsv.gz"), header=T, sep="\t", stringsAsFactors = F, com='')


virus_content_data = virus_content_data %>% rename(sample_name = X.sample)

# restrict to PE samples
virus_content_data = virus_content_data %>% filter(sample_name %in% PE_samples)
...
if (REMOVE_LIKELY_CONTAMINANTS) {
  ...
  # remove contaminants
  virus_content_data = left_join(virus_content_data, contaminants_info, by=c('sample_name', 'virus')) %>%  filter(is.na(CONTAM)) %>% select(-CONTAM)
