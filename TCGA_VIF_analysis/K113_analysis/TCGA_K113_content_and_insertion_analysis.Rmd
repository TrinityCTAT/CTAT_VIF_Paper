...
write.table(filtered_virus_content_data, "filtered_K113_content.tsv", sep="\t", row.names = F, quote=F)
...
# counts of samples containing K113
p_sample_counts = filtered_virus_content_data %>% select(TCGA, sample_name) %>% group_by(TCGA) %>% tally() %>% ggplot(aes(x=TCGA, y=n)) + geom_col() + 
...
p_mapped_dist = filtered_virus_content_data %>% select(TCGA, mapped) %>% ggplot(aes(x=TCGA, y=mapped)) + geom_boxplot()  +
        theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, size=rel(0.7) )) + scale_y_log10()

p = plot_grid(p_sample_counts, p_mapped_dist, ncol=1, align='v', rel_heights = c(0.25, 0.55))
    plot(p)
...
```{r}
# restrict analyses to PE samples

TCGA_rnaseq_sample_align_stats = read.table("../data/TCGA_rnaseq_sample_align_stats.tsv", header=T, sep="\t", stringsAsFactors = F)

PE_samples = TCGA_rnaseq_sample_align_stats %>% filter(num_fq_sets == 2) %>% pull(sample_name)
...
write.table(merged_distilled_report, file='tcga_subset.filtered_K113_content_and_insertions.distilled.tsv', quote=F, sep="\t", row.names=F)
...
ggsave(tcga_virus_summary_plot, file="tcga_K113_summary_plot.pdf", width=12, height=12)
...
# insertion sites per sample

```{r}

# upper limit on insertions / sample given thresholds

filtered_insertions_data %>% filter(is_primary != "False") %>% 
  group_by(sample_name) %>% tally() %>% arrange(desc(n))
...
# lower  limit on insertions / sample given thresholds

sample_insertion_count = filtered_insertions_data %>% filter(is_primary == "True") %>% 
  group_by(sample_name) %>% tally() %>% arrange(desc(n))

sample_insertion_count

```
```{r}

median(sample_insertion_count$n)
...
```{r}
# plot sample insertion prevalence
filtered_insertions_data %>% filter(is_primary == "True") %>% 
  group_by(sample_name) %>% tally(name='insertion_count') %>% ungroup() %>%
 group_by(insertion_count)  %>% tally(name='insertion_count_prevalence') %>%
  ggplot(aes(x=insertion_count, y=insertion_count_prevalence)) + geom_col() +
  ggtitle("K113 Insertion count sample prevalence")
...
# how do insertion counts in normals compare to tumors?

sample_insertion_count$participant = str_replace(sample_insertion_count$sample_name, "-\\w+$", "")
sample_insertion_count$tissue_type = sapply(sample_insertion_count$sample_name, function(x) { str_split(x, "-")[[1]][4] } )
sample_insertion_count_spread = sample_insertion_count %>% select(participant, tissue_type, n) %>%  spread(key=tissue_type, value=n, fill=NA) 
```

```{r}
sample_insertion_count_spread %>% filter(! is.na(NT) & ! is.na(TP) ) %>% gather(key=tissue_type, value=num_insertions, TP, NT) %>% ggplot(aes(x=tissue_type, y=num_insertions)) + geom_boxplot()

```

```{r}

NT_TP_paired_insertion_counts = sample_insertion_count_spread %>% filter(! is.na(NT) & ! is.na(TP) ) %>% select(NT, TP)

wilcox.test(NT_TP_paired_insertion_counts$NT, NT_TP_paired_insertion_counts$TP, paired=T)
...
# where are insertions happening on the K113 genome?

filtered_insertions_data %>% filter(is_primary == 'True') %>% select(sample_name, virus_brkend_grp, splice_type) %>% unique() %>% group_by(virus_brkend_grp, splice_type) %>% tally() %>% arrange(desc(n))

```

```{r}

filtered_insertions_data = filtered_insertions_data %>% mutate(virus_coord = ifelse(virus==chrA, coordA, coordB))
```

```{r}
filtered_insertions_data %>% filter(is_primary=='True') %>% select(sample_name, virus_coord) %>% group_by(virus_coord) %>% tally() %>%  
  ggplot() + geom_segment(aes(x=virus_coord, y=n, xend=virus_coord, yend=0)) + ggtitle("K113 breakpoint prevalence")
