...
# how many samples?
vif_insertions %>% select(sample_id) %>% unique() %>% nrow()
...
length(vif_hivid_samples_processed)
```
...
# restricts pub samples to those in vif_hivid_samples_processed!
...
# sum_vif_insertions  sum_pub_insertions
# 10891	2989	
...
combined_insertion_counts %>% select(sample_id) %>% unique() %>% nrow()

# 126 samples found by either pub or ctat-vif with insertions.
...
```{r}
# which samples had published = 0 but many in ctat-VIF?

combined_insertion_counts %>% filter(published_insertion_count == 0) %>% arrange(desc(vif_insertion_count))
...
# Examine sample T2122

vif_insertions %>% filter(sample_id == "T2122") %>% select(virus, virus_brkend_grp, total, total_rpm)
...
vif_insertions %>% filter(sample_id == "T2116") %>% select(virus, virus_brkend_grp, total, total_rpm)
...
# 935 agree at min 1 read support
# min reads 10 -> 75% agreement


# min_read_support
# num_insertions num_agree frac_agree
# 1	9315	935	0.1003757	
# 10	317	238	0.7507886	
