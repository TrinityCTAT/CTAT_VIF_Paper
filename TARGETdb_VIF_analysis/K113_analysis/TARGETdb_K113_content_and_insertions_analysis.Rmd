...
```{r}
virus_content_data = read.table("../K113.virus_content.tsv", header=T, sep="\t", stringsAsFactors = F)
...
insertions_data = insertions_data %>% mutate(sample_name = participant, sample_type = project) %>% select(-participant, -project)

virus_content_data = virus_content_data %>% mutate(sample_name = participant, sample_type = project) %>% select(-participant, -project)
...
p_sample_counts = filtered_virus_content_data %>% select(sample_type, sample_name) %>% unique() %>% group_by(sample_type) %>% tally() %>% ggplot(aes(x=sample_type, y=n)) + geom_col() + 
...
p_mapped_dist = filtered_virus_content_data %>% select(sample_type, mapped) %>% ggplot(aes(x=sample_type, y=mapped)) + geom_boxplot()  +
...
write.table(merged_distilled_report, file='TARGET.filtered_K113_content_and_insertions.distilled.tsv', quote=F, sep="\t", row.names=F)
...
```{r}
ggsave(virus_median_mapped_all_plot, file="TARGETdb_K113_median_mapped_all_plot.pdf", width=10, height=7)
