...
sample_info = read.table("data/TARGETdb.samples_list", sep="\t", header=T, stringsAsFactors = F)

sample_info = sample_info %>% rename(sample_name = sample_id, project = Project_ID, participant = participant_id, sample_type = Sample_Type) 

sample_info$project = str_replace(sample_info$project, "TARGET-", "")
sample_info$project = str_replace(sample_info$project, "ALL-P[\\d]", "ALL")
sample_info = sample_info %>% mutate(tumor_or_normal = ifelse(grepl("Normal", sample_type), "normal", "tumor"))

sample_info = sample_info %>% mutate(project = paste0(project, "-", tumor_or_normal) )
...
read_align_stats = read.table(gzfile("data/TARGET.star_align_stats.tsv.gz"), header=T, sep="\t", stringsAsFactors = F)
...
insertions_data = read.table(gzfile("data/TARGETdb.virus_insertions.tsv.gz"), sep="\t", stringsAsFactors = F, header=T, com='')
...
insertions_data = full_join(insertions_data, 
                            sample_info %>% select(sample_name, participant, project, sample_type), 
                            by='sample_name')
...
```{r}
virus_content_data = read.table(gzfile("data/TARGETdb.virus_content.tsv.gz"), header=T, sep="\t", stringsAsFactors = F, com='')

virus_content_data = virus_content_data %>% rename(sample_name = X.sample)


virus_content_data = full_join(virus_content_data, 
                            sample_info %>% select(sample_name, participant, project, sample_type), 
                            by='sample_name')
...
write.table(K113_virus_content_data, "K113.virus_content.tsv", quote=F, sep="\t", row.names=F)

```



# Merge samples for same participant

```{r}

insertions_data = insertions_data %>% mutate(sample = participant, sample_type = project) %>% select(-participant, -project)

virus_content_data = virus_content_data %>% mutate(sample = participant, sample_type = project) %>% select(-participant, -project)
...
sample_types = filtered_virus_content_data  %>% select(sample_type) %>% unique() %>% arrange(sample_type) %>% pull(sample_type)

TARGET_virus_content_plots = list()

counter = 1

for (sample_val in sample_types) {
    sample_type_data = filtered_virus_content_data %>% filter(sample_type == sample_val) 
    
    virus_counts =  sample_type_data %>% group_by(virus) %>% tally() %>% arrange(desc(n))
    
    ordered_viruses = virus_counts %>% pull(virus)
    
    sample_type_data$virus = factor(sample_type_data$virus, levels=ordered_viruses)
    
    p2 = sample_type_data %>% group_by(virus) %>% tally() %>% ggplot(aes(x=virus, y=n)) + geom_col() +
    ...
    p = sample_type_data %>% ggplot(aes(x=virus, y=mapped)) + geom_boxplot() + ggtitle(sample_val) +
    ...
    TARGET_virus_content_plots[[counter]] = p
    counter = counter + 1
}
...
pdf("TARGETdb_virus_content_plots.pdf")
for (i in seq(1, counter-1)) {
  plot(TARGET_virus_content_plots[[i]])
}
...
sample_counts = sample_info %>% select(participant, project) %>% unique() %>%  
  rename(sample_type = project) %>% 
  group_by(sample_type) %>% tally(name='tot_participants') 
...
sample_counts %>%
    ggplot(aes(x=sample_type, y=tot_participants)) + geom_bar(position='dodge', stat='identity') +
    geom_text(aes(label = tot_participants )  , vjust = 'left', color='black') +
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) + ylim(0,700)
...
merged_distilled_report = full_join(merged_distilled_report, 
                                    sample_info%>% 
                                        select(sample_name, sample_type), by=c('sample_name', 'sample_type') )
...
write.table(merged_distilled_report, file='TARGET.filtered_virus_content_and_insertions.distilled.tsv', quote=F, sep="\t", row.names=F)
...
sample_counts_with_virus = combined_data %>% filter(! is.na(mapped)) %>% 
    select(sample_name, sample_type) %>% unique() %>% 
...
summary_count_table = full_join(sample_counts, sample_counts_with_virus)
...
summary_count_table_w_frac = summary_count_table %>% 
    mutate(frac_w_virus = w_virus/tot_participants,
           frac_w_inserts = w_inserts / tot_participants)
...
summary_count_table_w_frac %>% gather(key=frac_type, value=fraction, frac_w_virus, frac_w_inserts) %>%
    ...
    ggtitle("TARGET sample virus content")
...
virus_median_mapped_by_tissue_id = filtered_virus_content_data %>% 
  select(sample_type, virus, mapped) %>% 
...
```{r}
ggsave(virus_median_mapped_all_plot, file="TARGETdb_virus_median_mapped_all_plot.pdf", width=10, height=7)

```

```{r}

virus_median_mapped_all_by_tumor_plot = virus_median_mapped_by_tissue_id  %>% ggplot(aes(y=virus, x=median_mapped, size=sample_count, color=sample_type)) +
  geom_point() +
  scale_x_log10()

virus_median_mapped_all_by_tumor_plot
```
...
virus_median_mapped_by_tissue_id_plot = virus_median_mapped_by_tissue_id  %>% 
  ggplot(aes(y=sample_type, x=median_mapped, size=sample_count, color=virus)) +
...
```{r}
ggsave(virus_median_mapped_by_tissue_id_plot, width=20, height=20, file="TARGETdb_virus_median_mapped_by_tissue_id.plot.pdf", limitsize = FALSE)
