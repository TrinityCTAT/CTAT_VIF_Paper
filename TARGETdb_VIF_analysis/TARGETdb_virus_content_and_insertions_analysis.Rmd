...
samples_processed = read.table("data/samples.processed", header=F, stringsAsFactors = F)[,1]

sample_info = read.table("data/target.samples.tsv", sep="\t", header=T, stringsAsFactors = F)

sample_info = sample_info %>% rename(sample_name = sample_id) %>% filter(sample_name %in% samples_processed)
...
insertions_data = read.table(gzfile("data/TARGET.vif.refined_results.agg.tsv.gz"), sep="\t", stringsAsFactors = F, header=T, com='')
...
insertions_data = full_join(insertions_data, 
                            sample_info %>% select(sample_name, Project_ID, Sample_Type), 
                            by='sample_name')
...
```{r}
virus_content_data = read.table(gzfile("data/TARGET.vif.virus_reports.agg.tsv.gz"), header=T, sep="\t", stringsAsFactors = F, com='')
...
for (sample_type in sample_types) {
    sample_type_data = filtered_virus_content_data %>% filter(Sample_Type == sample_type) 
    ...
    p = sample_type_data %>% ggplot(aes(x=virus, y=mapped)) + geom_boxplot() + ggtitle(sample_type) +
...
pdf("TARGET_virus_content_plots.pdf")
...
sample_counts = sample_info %>% group_by(Sample_Type) %>% tally(name='tot_samples')
