...
all_samples_processed = read.table("../all_samples_processed.tsv", header=T, sep="\t", stringsAsFactors = F) %>% filter(seqtype=='RNA')
...
sample_virus_counts = rnaseq_virus_content %>% filter(mapped >= 10 & n_bases_covered >= 500 & mapped_rpm >= 10) %>% 
...
sample_type_virus_fractions %>% mutate(project = paste(project, tumor_or_normal)) %>%
...
sample_type_virus_fractions %>% mutate(project = paste(project, tumor_or_normal)) %>% arrange(desc(frac_virus))  %>%
...
sample_type_virus_fractions %>% filter(project=="CESC" & tumor_or_normal=="tumor") %>% summarize(sum(frac_virus))
...
sample_types_restrict = sample_type_virus_fractions %>% 
  ...
  filter(frac_virus >= 0.02 & sample_virus_counts > 1) %>%
  select(cohort, project, tumor_or_normal, virus)
...
rnaseq_virus_content_restrict = left_join(sample_types_restrict,
                                          rnaseq_virus_content,
                                          by=c('cohort', 'project', 'tumor_or_normal', 'virus'))
...
rnaseq_virus_content_restrict %>%
  mutate(project = paste(cohort, project, tumor_or_normal)) %>%
  mutate(virus = ifelse(grepl("HPV", virus), "HPV", virus)) %>%
  #mutate(virus = ifelse(grepl("NC_007605|NC_009334", virus), "Eppstein-Barr_virus", virus) ) %>%
  filter(mapped_rpm >= 10 & n_bases_covered >= 500) %>%
  ggplot(aes(x=project, y=mapped_rpm, color=virus)) + geom_jitter() +
