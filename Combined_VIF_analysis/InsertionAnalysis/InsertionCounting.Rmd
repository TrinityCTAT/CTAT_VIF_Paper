...
merged_data = left_join(insertions_data,
                        virus_content_data %>% select(sample_name, virus, mapped, n_bases_covered, mapped_rpm, seqtype),
...
```{r}
project_sample_counts %>% ggplot(aes(x=cohort, y=participant_count, fill=seqtype)) + geom_col() + 
...
project_sample_counts %>%
  ...
  ggplot(aes(x=project, y=participant_count, fill=seqtype)) + geom_col() + 
