...
samples_in_rnaseq_and_other_seqtype = intersect(filtered_insertions_data %>% filter(seqtype == "RNA") %>% select(participant) %>% unique() %>% pull(participant),
...
# count oother seqtype insertions providing validations
...
matching_insertion_loci = matching_insertion_loci %>% select(rna_sample_name, virus, rna_insertion_locus, other_seqtype, other_insertion_locus, other_cohort) %>% unique()


matching_insertion_loci

message("number of matched rna insertion loci: ", nrow(matching_insertion_loci %>% select(rna_insertion_locus) %>% unique()))
