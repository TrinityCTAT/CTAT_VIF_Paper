...
# select the dominant isoform for each fusion 

nrow(FI_fusions_data)
FI_fusions_data = FI_fusions_data %>% group_by(sample_name, fusion_name) %>% arrange(desc(FFPM)) %>% filter(row_number() == 1) %>% ungroup()
nrow(FI_fusions_data)
```
...
fractional_cluster_att_representation = FI_fusions_data %>% group_by(fusion_name, fusion_cluster_att) %>% tally() %>% ungroup() %>%
