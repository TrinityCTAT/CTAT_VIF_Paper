...
NATGEN2015_hybcap_virus_content = NATGEN2015_hybcap_virus_content %>% mutate(tumor_or_normal = ifelse(participant %in% c('CASKID', 'S12', 'SIHA', 'HELA'), "tumor_cell_line", tumor_or_normal))
...
EBV_virus_content = EBV_virus_content %>%
  ...
  ungroup() %>%
  mutate(mapped = sum_mapped, n_bases_covered = max_n_bases_covered, frac_voered = max(frac_covered),
