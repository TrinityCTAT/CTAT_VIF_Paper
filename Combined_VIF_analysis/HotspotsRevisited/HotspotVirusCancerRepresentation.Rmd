...
all_insertions_data %>% filter(project=="CESC" & tumor_or_normal == "tumor") %>%
  filter(hotspot %in% c('chr8:127241438^CASC8', 'chr8:127627728^CASC11', 'chr8:127764585^PVT1')) %>%
...
all_insertions_data %>% filter(virus == "Eppstein-Barr") %>% select(project, cohort, participant) %>% unique() %>% 
...
all_insertions_data %>% filter(virus == "Eppstein-Barr") %>% 
  group_by(project, cohort) %>% tally() %>% ungroup() %>% mutate(frac=prop.table(n))
```
